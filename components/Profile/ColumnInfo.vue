...
.col.second
    ...
    .journal.block(v-if="this.$store.getters['admin/TASKS'].length > 0 && this.$store.getters.USER.role != 'user'")
        .title-wrapper 
            h2 Задания:
            NuxtLink(to="/educator/homework").btn.blue.sm Задания учеников
        .item-wrapper
            ProfileJournalItem(v-for="(item, index) in this.$store.getters['admin/TASKS']" :key="index" :item="item" type="admin")
